...
lookup_page (uint32_t *pd, const void *vaddr, bool create)
{
  ...
  pde = pd + pd_no (vaddr);
  if (*pde == 0) //if table is not pointed
    {
      if (create)
        {
...
/* Adds a mapping in page directory PD from user virtual page
   ...
   failed. */
bool new_pagedir_set_page(uint32_t *pd, void *upage, struct frame *frame, bool writable)
{
	uint32_t *pte;
...
bool
new_install_page (void *upage, struct frame *frame, bool writable)
{
  ...
  if (!new_pagedir_set_page (t->pagedir, upage, frame, writable)) {
